...
int main(int argc, char* argv[])
{
	...
	while(1)
	{
		...
		if(ntohs(ether_cover->type) == 0x0806)
		{
			...
			{
				inet_ntop(AF_INET, arp_cover->proto_src, (char*)victim_ip, sizeof(victim_ip));		//20 is ok?
				printf("%s\n", victim_ip);
				if(!strncmp((const char*)victim_ip, argv[1], strlen(argv[1])))
				{
					printf("OK\n");	
					for(i = 0; i < 6; i++)
	...
	while(pcap_sendpacket(handle, send_packet, sizeof(ethernet)+sizeof(arp)) != 0)
	{
		printf("Can't send the packet!\n");
		return -1;
	}
	char *relay;
	ether_h *relay_ether = (ether_h*)malloc(sizeof(ethernet));
	while(1)
	{
		is_ok = pcap_next_ex(handle, &header, &packet);
		if( is_ok == 0 )
		...
		memcpy(relay_ether, relay, sizeof(ethernet));
		cout << "Copied" << endl;
		relay_ether = (ether_h*)relay;
		for( i = 0; i < 6; i++)
		{
			relay_ether->src[i] = my_mac[i];
		}
		//ADD convert dst mac to target's mac address
		if(pcap_sendpacket(handle, send_packet, header->len) != 0)
			cout << "Can't send the packet!\n" << endl;
		else
		{
			for (int k = 0; k < 6; k++)
